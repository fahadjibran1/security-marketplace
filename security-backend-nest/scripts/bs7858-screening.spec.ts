...
function record(overrides:Partial<GuardScreening>={}){return Object.assign({id:1,status:ScreeningStatus.IN_PROGRESS,screeningPeriodYears:5,legalFullName:'Candidate',dateOfBirth:'1990-01-01',nationality:'British',currentAddress:'Address',history:[{startDate:ago(5),endDate:null,isCurrent:true}],addresses:[{startDate:ago(5),endDate:null,isCurrent:true,verificationState:VerificationState.VERIFIED}],references:[{status:ReferenceStatus.VERIFIED,sourceVerified:true}],evidence:['identity','address','sia','right_to_work'].map(category=>({category,uploadCompletedAt:new Date()})),consents:[{withdrawnAt:null}],exceptions:[],identityVerification:VerificationState.VERIFIED,siaRegisterVerification:VerificationState.VERIFIED,rightToWorkVerification:VerificationState.VERIFIED},overrides) as GuardScreening;}
async function main(){
 ...
 await test('continuous five-year address coverage permits submission',()=>assert.equal(svc.requirements(record()).addressChronology.continuous,true));
 await test('missing identity evidence prevents submission',()=>assert.ok(svc.requirements(record({evidence:[]})).missing.some((x:string)=>x.includes('Identity evidence'))));
 await test('missing address evidence prevents submission',()=>assert.ok(svc.requirements(record({evidence:[]})).missing.some((x:string)=>x.includes('Address evidence'))));
 await test('missing SIA evidence prevents submission',()=>assert.ok(svc.requirements(record({evidence:[]})).missing.some((x:string)=>x.includes('SIA evidence'))));
 await test('missing Right to Work evidence prevents submission',()=>assert.ok(svc.requirements(record({evidence:[]})).missing.some((x:string)=>x.includes('Right to Work evidence'))));
