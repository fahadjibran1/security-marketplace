...
test('Fix-this navigation reaches server-selected step and opens correction form',()=>assert(panel.includes('onFix(item.step)')&&panel.includes('navigateToRemediation(target as Step)')&&panel.includes('if(next==="addresses")')&&panel.includes('if(next==="history")'),'fix navigation missing'));
...
test('Company cannot bypass its own compliance requirements',()=>assert(compliance.includes('assertGuardAssignable')&&compliance.includes('getBlockingReasons'),'company bypass detected'));
test('S4 screening is not a deployment gate',()=>assert(!compliance.includes('this.screeningService.isGuardVetted'),'screening reintroduced as an assignability gate'));
