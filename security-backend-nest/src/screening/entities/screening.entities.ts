...
export enum ReferenceStatus { NOT_REQUESTED='NOT_REQUESTED', REQUESTED='REQUESTED', RECEIVED='RECEIVED', SOURCE_VERIFICATION_REQUIRED='SOURCE_VERIFICATION_REQUIRED', VERIFIED='VERIFIED', DISCREPANCY='DISCREPANCY', UNABLE_TO_VERIFY='UNABLE_TO_VERIFY', REJECTED='REJECTED' }
...
export class GuardScreening {
  ...
  @Column({ type: 'enum', enum: VerificationState, enumName: 'screening_verification_state_enum', default: VerificationState.UNVERIFIED }) identityVerification!: VerificationState;
  ...
  @Column({ type: 'enum', enum: VerificationState, enumName: 'screening_verification_state_enum', default: VerificationState.UNVERIFIED }) siaRegisterVerification!: VerificationState;
  ...
  @Column({ type: 'enum', enum: VerificationState, enumName: 'screening_verification_state_enum', default: VerificationState.UNVERIFIED }) rightToWorkVerification!: VerificationState;
...
export class ScreeningHistory {
  @PrimaryGeneratedColumn() id!: number;
  @ManyToOne(() => GuardScreening, (screening) => screening.history, { nullable: false, onDelete: 'CASCADE' }) @JoinColumn({ name: 'screeningId' }) screening!: GuardScreening;
  ...
  @Column({ type: 'enum', enum: VerificationState, enumName: 'screening_verification_state_enum', default: VerificationState.UNVERIFIED }) verificationState!: VerificationState;
  @CreateDateColumn() createdAt!: Date; @UpdateDateColumn() updatedAt!: Date;
}

@Entity('screening_addresses')
export class ScreeningAddress { @PrimaryGeneratedColumn() id!: number; @ManyToOne(() => GuardScreening, (s) => s.addresses, { nullable: false, onDelete: 'CASCADE' }) @JoinColumn({ name: 'screeningId' }) screening!: GuardScreening; @Column({ type:'text' }) address!: string; @Column({ type:'varchar', length:200, nullable:true }) addressLine1?:string|null; @Column({ type:'varchar', length:200, nullable:true }) addressLine2?:string|null; @Column({ type:'varchar', length:150, nullable:true }) townCity?:string|null; @Column({ type:'varchar', length:8, nullable:true }) postcode?:string|null; @Column({ type:'date' }) startDate!: string; @Column({ type:'date', nullable:true }) endDate?: string|null; @Column({ default:false }) isCurrent!: boolean; @Column({ type:'enum', enum:VerificationState, enumName:'screening_verification_state_enum', default:VerificationState.UNVERIFIED }) verificationState!: VerificationState; @CreateDateColumn() createdAt!: Date; }

@Entity('screening_references')
export class ScreeningReference { @PrimaryGeneratedColumn() id!: number; @ManyToOne(() => GuardScreening, (s) => s.references, { nullable:false, onDelete:'CASCADE' }) @JoinColumn({name:'screeningId'}) screening!: GuardScreening; @ManyToOne(() => ScreeningHistory, { nullable:false, onDelete:'CASCADE' }) @JoinColumn({name:'historyId'}) history!: ScreeningHistory; @Column() organisation!: string; @Column() contactPerson!: string; @Column() relationship!: string; @Column() businessEmail!: string; @Column({type:'varchar',nullable:true}) phone?:string|null; @Column({type:'text',nullable:true}) postalDetails?:string|null; @Column({type:'enum',enum:ReferenceStatus,enumName:'screening_reference_status_enum',default:ReferenceStatus.NOT_REQUESTED}) status!:ReferenceStatus; @Column({type:'timestamp',nullable:true}) requestedAt?:Date|null; @Column({type:'timestamp',nullable:true}) receivedAt?:Date|null; @Column({type:'varchar',nullable:true}) verificationMethod?:string|null; @Column({default:false}) sourceVerified!:boolean; @Column({type:'int',nullable:true}) verifiedByUserId?:number|null; @Column({type:'timestamp',nullable:true}) verifiedAt?:Date|null; @Column({type:'text',nullable:true,select:false}) outcomeNotes?:string|null; @Column({type:'date',nullable:true}) confirmedStartDate?:string|null; @Column({type:'date',nullable:true}) confirmedEndDate?:string|null; @Column({type:'boolean',nullable:true}) confirmedIsCurrent?:boolean|null; @CreateDateColumn() createdAt!:Date; @UpdateDateColumn() updatedAt!:Date; }

@Entity('screening_evidence')
export class ScreeningEvidence { @PrimaryGeneratedColumn() id!:number; @ManyToOne(() => GuardScreening,(s)=>s.evidence,{nullable:false,onDelete:'CASCADE'}) @JoinColumn({name:'screeningId'}) screening!:GuardScreening; @Column({type:'enum',enum:EvidenceCategory}) category!:EvidenceCategory; @Column() storageProvider!:string; @Column({unique:true}) storageKey!:string; @Column() originalFileName!:string; @Column() mimeType!:string; @Column({type:'bigint'}) sizeBytes!:string; @Column({type:'timestamp',nullable:true}) uploadCompletedAt?:Date|null; @Column({type:'enum',enum:VerificationState,enumName:'screening_verification_state_enum',default:VerificationState.UNVERIFIED}) verificationState!:VerificationState; @Column() uploadedByUserId!:number; @Column({type:'int',nullable:true}) verifiedByUserId?:number|null; @Column({type:'timestamp',nullable:true}) verifiedAt?:Date|null; @CreateDateColumn() createdAt!:Date; }

@Entity('screening_consents')
export class ScreeningConsent { @PrimaryGeneratedColumn() id!:number; @ManyToOne(() => GuardScreening,(s)=>s.consents,{nullable:false,onDelete:'CASCADE'}) @JoinColumn({name:'screeningId'}) screening!:GuardScreening; @Column() consentVersion!:string; @Column() candidateUserId!:number; @Column({type:'timestamp'}) acceptedAt!:Date; @Column({type:'timestamp',nullable:true}) withdrawnAt?:Date|null; @CreateDateColumn() createdAt!:Date; }

@Entity('screening_exceptions')
export class ScreeningException { @PrimaryGeneratedColumn() id!:number; @ManyToOne(() => GuardScreening,(s)=>s.exceptions,{nullable:false,onDelete:'CASCADE'}) @JoinColumn({name:'screeningId'}) screening!:GuardScreening; @Column() code!:string; @Column({type:'text'}) description!:string; @Column({default:false}) resolved!:boolean; @Column({type:'int',nullable:true}) resolvedByUserId?:number|null; @Column({type:'timestamp',nullable:true}) resolvedAt?:Date|null; @CreateDateColumn() createdAt!:Date; }
