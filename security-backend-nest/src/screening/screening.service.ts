...
export class ScreeningService {
  ...
  private requirements(s:GuardScreening,review=false){
    ...
    if(review){if(s.identityVerification!==VerificationState.VERIFIED)missing.push('Identity verification is incomplete.');if(!currentAddress||currentAddress.verificationState!==VerificationState.VERIFIED)missing.push('Current address verification is incomplete.');if(s.siaRegisterVerification!==VerificationState.VERIFIED)missing.push('SIA register verification is incomplete.');if(s.rightToWorkVerification!==VerificationState.VERIFIED)missing.push('Right to Work verification is incomplete.');if(!verifiedReference)missing.push('A source-verified reference is required.');if(!s.guard?.siaExpiryDate)missing.push('SIA licence expiry is required.');if(!s.guard?.rightToWorkStatus)missing.push('Right to Work information is required.');for(const [category,label] of [['identity','Identity'],['address','Address'],['sia','SIA'],['right_to_work','Right to Work']] as const){const records=uploadedEvidence(category);if(records.length&&!records.some(e=>e.verificationState===VerificationState.VERIFIED))missing.push(`${label} evidence has not been verified.`);}}
