...
export function reviewScreeningReference(screeningId:number,referenceId:number,payload:{status:'VERIFIED'|'UNABLE_TO_VERIFY'|'REJECTED'|'SOURCE_VERIFICATION_REQUIRED';verificationMethod:string;notes:string;confirmed:true}){return request<GuardScreening>(`/screening/${screeningId}/references/${referenceId}/review`,{method:'PATCH',body:JSON.stringify(payload)});}
