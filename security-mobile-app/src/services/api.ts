...
export function updateMyScreeningAddress(id:number,payload:{address?:string;addressLine1?:string;addressLine2?:string;townCity?:string;postcode?:string;startDate:string;endDate?:string;isCurrent:boolean}){return request<GuardScreening>(`/screening/mine/addresses/${id}`,{method:'PUT',body:JSON.stringify(payload)});}
export function deleteMyScreeningAddress(id:number){return request<GuardScreening>(`/screening/mine/addresses/${id}`,{method:'DELETE'});}
...
export function verifyScreeningCheck(id:number,check:'identity'|'address'|'sia'|'rtw',evidenceId:number,state:'VERIFIED'|'REJECTED'='VERIFIED',method?:string){return request<GuardScreening>(`/screening/${id}/checks/${check}`,{method:'PATCH',body:JSON.stringify({state,method:method||(state==='REJECTED'?'Authorised reviewer rejected the evidence':'Authorised reviewer evidence inspection'),evidenceId})});}
export function requestScreeningReference(screeningId:number,referenceId:number){return request(`/screening/${screeningId}/references/${referenceId}/request`,{method:'POST'});}
export function reviewScreeningReference(screeningId:number,referenceId:number,payload:{status:'VERIFIED'|'DISCREPANCY'|'UNABLE_TO_VERIFY'|'REJECTED'|'SOURCE_VERIFICATION_REQUIRED';verificationMethod:string;notes:string;confirmed:true;confirmedStartDate?:string;confirmedEndDate?:string;confirmedIsCurrent?:boolean}){return request<GuardScreening>(`/screening/${screeningId}/references/${referenceId}/review`,{method:'PATCH',body:JSON.stringify(payload)});}
