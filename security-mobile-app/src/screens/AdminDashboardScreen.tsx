...
import {
  ...
  formatApiErrorMessage, getScreening, listScreenings,
...
export function AdminDashboardScreen() {
  ...
  useEffect(() => { setQuery(''); setSelected(null); setReviewReason(''); setReviewFeedback(null); load(); }, [load]);
  ...
  const runReviewAction=async(key:string,mutation:()=>Promise<unknown>,success:string,reasonRequired=false)=>{if(reviewAction||!selected)return;if(reasonRequired&&!reviewReason.trim()){setReviewFeedback({tone:'error',message:'A review reason is required for this action.'});return;}const selectedId=Number(selected.id);setReviewAction(key);setReviewFeedback(null);try{await mutation();await refreshSelectedScreening(selectedId);setReviewFeedback({tone:'success',message:success});}catch(actionError){setReviewFeedback({tone:'error',message:formatApiErrorMessage(actionError,'Unable to update this screening review.')});}finally{setReviewAction(null);}};
  ...
  return <View style={[styles.shell, compact && styles.shellCompact]}>{navigation}<ScrollView style={styles.content} contentContainerStyle={styles.contentInner}>
    ...
    {selected ? <View style={styles.detailPanel}><Text style={styles.detailHeading}>{selected.title}</Text><Text style={styles.rowDetail}>{selected.detail}</Text>{selected.status ? <Text style={styles.status}>{selected.status}</Text> : null}
      ...
      <TextInput accessibilityLabel="Screening review reason" placeholder="Required review decision or information-request reason" value={reviewReason} onChangeText={setReviewReason} style={styles.search}/><Text style={styles.reasonHelp}>A reason is required when requesting information, completing, rejecting or expiring a screening.</Text><View style={styles.grid}>
        ...
        {selected.status==='UNDER_REVIEW'?(['identity','address','sia','rtw'] as const).map(check=><Pressable key={check} disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={()=>runReviewAction(`verify-${check}`,()=>verifyScreeningCheck(Number(selected.id),check),`${check==='rtw'?'Right to Work':check[0].toUpperCase()+check.slice(1)} verified successfully.`)}><Text style={styles.approveText}>{reviewAction===`verify-${check}`?'Verifying…':`Verify ${check.toUpperCase()}`}</Text></Pressable>):null}
        {['READY_FOR_REVIEW','UNDER_REVIEW'].includes(selected.status||'')?<Pressable disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={()=>runReviewAction('request',()=>requestScreeningInformation(Number(selected.id),reviewReason.trim()),'Information request recorded successfully.',true)}><Text style={styles.approveText}>{reviewAction==='request'?'Requesting…':'Request information'}</Text></Pressable>:null}
        {selected.status==='UNDER_REVIEW'?<Pressable disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={()=>runReviewAction('complete',()=>completeScreeningReview(Number(selected.id),reviewReason.trim()),'Screening completed successfully.',true)}><Text style={styles.approveText}>{reviewAction==='complete'?'Completing…':'Complete screening'}</Text></Pressable>:null}
        {['READY_FOR_REVIEW','UNDER_REVIEW'].includes(selected.status||'')?<Pressable disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={()=>runReviewAction('reject',()=>rejectScreening(Number(selected.id),reviewReason.trim()),'Screening rejected successfully.',true)}><Text style={styles.approveText}>{reviewAction==='reject'?'Rejecting…':'Reject'}</Text></Pressable>:null}
        {selected.status==='VETTED'?<Pressable disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={()=>runReviewAction('expire',()=>expireScreening(Number(selected.id),reviewReason.trim()),'Screening marked expired successfully.',true)}><Text style={styles.approveText}>{reviewAction==='expire'?'Expiring…':'Mark expired'}</Text></Pressable>:null}
      </View></>:null}<Pressable onPress={() => setSelected(null)}><Text style={styles.retry}>Close detail</Text></Pressable></View> : null}
    {section !== 'overview' && !loading && !error ? <View style={styles.list}>{visibleRows.map((row) => <Pressable accessibilityRole="button" accessibilityLabel={`Open ${row.title} detail`} onPress={() => {setSelected(row);setReviewReason('');setReviewFeedback(null);}} key={row.id} style={styles.row}><View style={styles.rowCopy}><Text style={styles.rowTitle}>{row.title}</Text><Text style={styles.rowDetail}>{row.detail}</Text></View>{row.status ? <Text style={styles.status}>{row.status}</Text> : null}</Pressable>)}</View> : null}
  </ScrollView></View>;
}
