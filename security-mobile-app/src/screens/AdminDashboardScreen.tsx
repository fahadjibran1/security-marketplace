...
import type { ScreeningQueueFilter, ScreeningQueueResponse, ScreeningQueueRow } from '../types/models';
...
export function AdminDashboardScreen() {
  ...
  const openQueueRow=useCallback(async(row:ScreeningQueueRow)=>{
    setReviewReason('');setReviewReasonAction(null);setReviewFeedback(null);setReviewCategory(null);setInspectedEvidenceId(null);setVerificationConfirm(false);setReferencesOpen(false);setReferenceReviewId(null);setCompletedOpen(false);
    setLoading(true);
  ...
  const backToQueue=useCallback(()=>{setSelected(null);setReviewCategory(null);setReferencesOpen(false);setReviewFeedback(null);setCompletedOpen(false);void loadQueue().catch(()=>undefined);},[loadQueue]);
  ...
  const classification=selected?.raw?.reviewClassification as {reviewerActions:Array<{key:string;label:string;message:string}>;guardActions:Array<{key:string;label:string;message:string}>;bucket:string;ready:boolean;informationRequestOutstanding:boolean}|undefined;
  ...
  // While a specific check is being reviewed, the unrelated sections step out of the way.
  ...
  return <View style={[styles.shell, compact && styles.shellCompact]}>{imageViewer?<Modal transparent animationType="fade" onRequestClose={()=>setImageViewer(null)}><View style={styles.modalBackdrop}><View style={styles.imageModal}><Text style={styles.detailHeading}>{imageViewer.label}</Text><Image source={{uri:imageViewer.url}} resizeMode="contain" style={styles.evidenceImage}/><Text style={styles.rowDetail}>This short-lived private preview does not change verification state.</Text><Pressable style={styles.approveButton} onPress={()=>setImageViewer(null)}><Text style={styles.approveText}>Close preview</Text></Pressable></View></View></Modal>:null}{navigation}<ScrollView ref={contentScroll} style={styles.content} contentContainerStyle={styles.contentInner}>
    ...
    {selected ? <View style={styles.detailPanel}>{section==='screening'?<Pressable accessibilityRole="button" onPress={backToQueue}><Text style={styles.backToQueue}>← Back to Screening Review</Text></Pressable>:null}<Text style={styles.detailHeading}>{selected.title}</Text><Text style={styles.rowDetail}>{selected.detail}</Text>{selected.status ? <Text style={styles.status}>{selected.status}</Text> : null}
      {section==='screening'?<><View style={styles.readinessReady}><Text style={styles.reviewSuccessTitle}>SCREENING REVIEW</Text>
        {/* A four-line summary only. The per-check breakdown lives in YOUR ACTIONS below; repeating
            it here is what made the old panel a wall of text. */}
        <Text style={styles.rowTitle}>Candidate information: {text(selected.raw?.progress,'0')}% complete{Number(selected.raw?.progress)===100?' ✓':''}</Text>
        <Text style={styles.rowDetail}>Verification: {verificationSummary?`${verificationSummary.completed} of ${verificationSummary.total} checks complete`:'Loading authoritative checks'}</Text>
        <Text style={styles.rowDetail}>Reviewer actions remaining: {yourActions.length}</Text>
        <Text style={styles.rowDetail}>Guard actions remaining: {waitingForGuard.length}</Text>
        {classification?.informationRequestOutstanding?<Text style={styles.rowDetail}>An information request was sent to this Guard and has not been answered{selected.raw?.reviewNotes?`: “${text(selected.raw.reviewNotes)}”`:'.'}</Text>:null}</View>
      ...
      {/* Choosing an action focuses it: the other lists step aside rather than making the reviewer
          scroll past them. */}
      {!focusedReview?<><Text style={styles.detailHeading}>YOUR ACTIONS</Text>
      {yourActions.length?yourActions.map(check=><View key={check.key} style={styles.attentionRow}><View style={styles.rowCopy}><Text style={styles.rowTitle}>{check.label}</Text><Text style={styles.rowDetail}>{text(check.message,'Awaiting your verification.')}</Text></View>{check.key==='reference'?<Pressable disabled={!!reviewAction} style={[styles.approveButton,reviewAction&&styles.disabledButton]} onPress={openReferences}><Text style={styles.approveText}>Review reference</Text></Pressable>:<Pressable disabled={!!reviewAction||selected.status!=='UNDER_REVIEW'} style={[styles.approveButton,(reviewAction||selected.status!=='UNDER_REVIEW')&&styles.disabledButton]} onPress={()=>{setReviewCategory(check.key as 'identity'|'address'|'sia'|'rtw');setInspectedEvidenceId(null);setVerificationConfirm(false);setReviewFeedback(null);}}><Text style={styles.approveText}>Review {check.label}</Text></Pressable>}</View>):<Text style={styles.rowDetail}>{selected.status==='READY_FOR_REVIEW'?'Start the review to begin verifying this Guard.':'Nothing is waiting on you for this Guard.'}</Text>}
      {/* Zero Guard actions is a one-line fact, not a section worth a heading and a paragraph. */}
      {waitingForGuard.length?<><Text style={styles.detailHeading}>WAITING FOR GUARD</Text>
      {waitingForGuard.map(entry=><View key={entry.key} style={styles.waitingRow}><Text style={styles.waitingLabel}>Waiting for Guard</Text><Text style={styles.rowTitle}>{entry.label}</Text><Text style={styles.rowDetail}>{entry.message}</Text></View>)}</>:<Text style={styles.rowDetail}>No Guard action required.</Text>}
      ...
      <Pressable accessibilityRole="button" accessibilityState={{expanded:completedOpen}} onPress={()=>setCompletedOpen(value=>!value)}><Text style={styles.collapseToggle}>{completedOpen?'▾':'▸'} Completed checks ({completedChecks.length})</Text></Pressable></>:null}
