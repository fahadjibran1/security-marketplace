  ...
  return <View style={[styles.shell, compact && styles.shellCompact]}>{navigation}<ScrollView style={styles.content} contentContainerStyle={styles.contentInner}>
    ...
    {selected ? <View style={styles.detailPanel}><Text style={styles.detailHeading}>{selected.title}</Text><Text style={styles.rowDetail}>{selected.detail}</Text>{selected.status ? <Text style={styles.status}>{selected.status}</Text> : null}
      {section==='screening'?<><View style={styles.screeningReviewGrid}>
        {([['Identity',selected.raw?.identityVerification],['SIA register',selected.raw?.siaRegisterVerification],['Right to Work',selected.raw?.rightToWorkVerification],['Consent',selected.raw?.consents?.some((entry:Record<string,unknown>)=>!entry.withdrawnAt)?'CURRENT':'MISSING']] as Array<[string,unknown]>).map(([label,value])=><View key={label} style={styles.screeningCheck}><Text style={styles.cardLabel}>{label}</Text><Text style={styles.screeningCheckValue}>{text(value,'Not supplied').replaceAll('_',' ')}</Text></View>)}
      </View>
      ...
      <TextInput accessibilityLabel="Screening review reason" placeholder="Required review decision or information-request reason" value={reviewReason} onChangeText={setReviewReason} style={styles.search}/><View style={styles.grid}>
